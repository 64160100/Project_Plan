...
function toggleDateForm(element) {
    ...
    const expenseStatus = document.querySelector('input[name="Expense_Status"]:checked');
    if (expenseStatus) {
        toggleDateForm(expenseStatus);
    }
    
    // ตรวจสอบว่าเลือกหลายวัน (M) หรือไม่
    const isMultipleDays = element.value === 'M' || element.id === 'multiple_days';
    
    if (isMultipleDays) {
        // หลายวัน - แสดงปุ่มเพิ่มวัน
        if (addBudgetFormBtn) {
            addBudgetFormBtn.style.display = 'block';
        }
        
        // ถ้ามีหลายฟอร์มให้แสดงปุ่มลบ
        ...
        // วันเดียว - ซ่อนปุ่มเพิ่มวัน และปุ่มลบทุกปุ่ม
        if (addBudgetFormBtn) {
            // Only hide if we're in single day mode
            if (element.checked && element.id === 'single_day') {
                addBudgetFormBtn.style.display = 'none';
            } else if (element.id === 'single_day' && element.checked) {
                // If we're loading with single_day and checked, hide the button
                addBudgetFormBtn.style.display = 'none';
            } else if (element.id === 'multiple_days' && element.checked) {
                // If multiple_days is checked, always show the button
                addBudgetFormBtn.style.display = 'block';
            }
        }
        ...
        // ตรวจสอบว่ามีการเลือก "วันเดียว" จากในฟอร์ม แต่ไม่ใช่เมื่อโหลดหน้าครั้งแรก
        if (element.checked && element.id === 'single_day') {
            // เก็บฟอร์มแรกไว้ (สำหรับวันเดียว)
            const firstForm = document.querySelector('.budget-form');
    ...
    // อัพเดทการคำนวณยอดรวมหลังจากเปลี่ยนแปลงฟอร์ม
    setTimeout(() => {
