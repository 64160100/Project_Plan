...
<head>
    <link rel="stylesheet" href="{{ asset('css/storageFiles.css') }}">
...
<div class="file-storage-container">
    ...
    <div class="header-container">
        <a href="{{ route('proposeProject') }}" class="back-btn1">
    ...
    <div class="row">
        <!-- Upload Form -->
        @if($employee->IsAdmin === 'Y' || ($employee->IsResponsible === 'Y' && $employee->Id_Employee ===
        $project->Employee_Id))
        <div class="col-md-4">
        ...
        <!-- Files Table -->
        <div
            class="col-md-{{ $employee->IsAdmin === 'Y' || ($employee->IsResponsible === 'Y' && $employee->Id_Employee === $project->Employee_Id) ? '8' : '12' }}">
                    ...
                    <div class="table-responsive">
                        <table class="file-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        @if($employee->IsAdmin === 'Y' || ($employee->IsResponsible === 'Y' &&
                                        $employee->Id_Employee === $project->Employee_Id))
                                        <span class="file-name"
                                            data-id="{{ $file->Id_Storage_File }}">{{ $file->Name_Storage_File }}</span>
                                        <input type="text" class="form-control file-name-input d-none"
                                            data-id="{{ $file->Id_Storage_File }}"
                                            value="{{ $file->Name_Storage_File }}">
                                        @else
                                        <span>{{ $file->Name_Storage_File }}</span>
                                        @endif
                                    </td>
                                    <td>
                                        <span class="file-type-badge">{{ $file->Type_Storage_File }}</span>
                                    </td>
                                    <td>
                                        <span class="file-size-badge">{{ $file->getHumanReadableSize() }}</span>
                                    </td>
                                    <td>
...
function saveFileName() {
    ...
    fetch('{{ route("StorageFiles.updateName") }}', {
            method: 'POST',
            headers: {
