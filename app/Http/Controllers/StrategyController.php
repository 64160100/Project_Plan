...
class StrategyController extends Controller
{
    ...
    {
        try {
            $strategy = Strategy::where('Strategic_Id', $Id_Strategic)->get();
            $strategic = Strategic::with('strategies.kpis')->findOrFail($Id_Strategic);
    ...
    {
        $strategy = Strategy::findOrFail($Id_Strategy);
    ...
    {
        $strategies = Strategy::with('strategicObjectives')->where('Strategic_Id', $Id_Strategy)->get();
    ...
    {
        $strategy = Strategy::with('kpis')->findOrFail($Id_Strategy);
    ...
    public function updateStrategy(Request $request, $Id_Strategy)
    {
        ...
        if ($request->has('kpis') && is_array($request->kpis)) {
            foreach ($request->kpis as $Id_Kpi => $kpiData) {
                if (is_null($kpiData['Name_Kpi']) || is_null($kpiData['Target_Value'])) {
                    KpiModel::findOrFail($Id_Kpi)->delete();
                } else {
        ...
        if ($request->has('strategicObjectives') && is_array($request->strategicObjectives)) {
            foreach ($request->strategicObjectives as $Id_Strategic_Objectives => $SOData) {
                if (is_null($SOData['Details_Strategic_Objectives'])) {
                    StrategicObjectivesModel::findOrFail($Id_Strategic_Objectives)->delete();
                } else {
